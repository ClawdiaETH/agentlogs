...
export async function fetchGitHubData(githubUsername?: string): Promise<GitHubData> {
  const empty: GitHubData = { commits: [], commitCount: 0, reposActive: [], peakHourFromCommits: null };

  if (!githubUsername) return empty;

  try {
    const headers: Record<string, string> = { 'User-Agent': 'agentsea-assembler' };
