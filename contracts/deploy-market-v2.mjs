#!/usr/bin/env node
/**
 * Deploy AgentMarketV2.sol to Base mainnet.
 *
 * Usage: node deploy-market-v2.mjs [--testnet] [--verify]
 *
...
function loadArtifact() {
  const artifactPath = path.join(__dirname, 'artifacts/src/AgentMarketV2.sol/AgentMarketV2.json');
...
async function main() {
  console.log(`Deploying AgentMarketV2 to ${TESTNET ? 'Base Sepolia testnet' : 'Base mainnet'}...\n`);
  ...
  console.log(`\nAgentMarketV2 deployed at: ${address}\n`);

  console.log(`${'='.repeat(50)}`);
  console.log(`AgentMarketV2 deployed at: ${address}`);
  console.log(`Network: ${TESTNET ? 'Base Sepolia' : 'Base mainnet'}`);
  console.log(`Treasury: ${TREASURY}`);
  console.log(`Fee: 2.5% (250 bps)`);
  console.log(`Offer duration: 7 days`);
  console.log(`${'='.repeat(50)}\n`);

  const abiPath = path.join(__dirname, 'AgentMarketV2.abi.json');
  ...
  if (VERIFY) {
    ...
    if (!apiKey) {
      ...
      try {
        ...
        console.log(`Retry: cd contracts && BASESCAN_API_KEY=$BASESCAN_API_KEY npx hardhat verify --network base ${address} "${TREASURY}"`);
      }
    }
  }

  console.log('\nNext steps:');
  console.log(`  1. Set NEXT_PUBLIC_MARKET_V2_CONTRACT=${address} in your .env`);
  console.log(`  2. Register agent wallet: contract.registerAgent("0xf17b5dD382B048Ff4c05c1C9e4E24cfC5C6adAd9")`);
